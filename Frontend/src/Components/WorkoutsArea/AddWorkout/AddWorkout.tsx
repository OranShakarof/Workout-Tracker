...
function AddWorkout(): JSX.Element {
  const { register, handleSubmit, formState: { errors }, trigger, control } = useForm<WorkoutModel>();

  const { fields: exercises, append: appendExercise, remove: removeExercise } = useFieldArray<WorkoutModel, "exercises">({ control, name: "exercises" });
  ...
  async function send(workout: WorkoutModel) {
    try {
      ...
      await workoutsService.addWorkout(workout);
      notifyService.success("Workout has been added successfully!");
      navigate("/workouts");
    } catch (err: any) {
        const responseData = err.response.data;
        if (responseData && responseData.includes('E11000 duplicate key error')) {
            notifyService.error("A workout already exists for this date.");
        } 
        else notifyService.error(err);
    }
  }
    ...
    <div className="AddWorkout">
      <form onSubmit={handleSubmit(send)}>
        <Grid container className="Box">
                ...
                <Grid item xs={6}>
                  <TextField
                    ...
                    {...register(`exercises.${exerciseIndex}.exercise`, {
                        ...
                        maxLength: {
                            value: 100,
                            message: "* Exercise name need to be less then 100 Chars"
                        }
                    })}
                    onBlur={() => trigger(`exercises.${exerciseIndex}.exercise`)}
                    error={!!errors.exercises?.[exerciseIndex]?.exercise}
                    helperText={errors.exercises?.[exerciseIndex]?.exercise?.message}
                  />
                </Grid>
                <Grid item xs={6}>
                  <TextField
              ...
              {exercise.sets.map((set, setIndex) => (
                <Grid container spacing={2} key={setIndex}>
                  ...
                  <Grid item xs={6} sm={3}>
                    ...
                    onBlur={() => trigger(`exercises.${exerciseIndex}.sets.${setIndex}.weight`)}
                    error={!!errors.exercises?.[exerciseIndex]?.sets?.[setIndex]?.weight}
                    helperText={errors.exercises?.[exerciseIndex]?.sets?.[setIndex]?.weight?.message}
                    />
                  </Grid>
                  <Grid item xs={6} sm={3}>
                    <TextField
                    ...
                    onBlur={() => trigger(`exercises.${exerciseIndex}.sets.${setIndex}.reps`)}
                    error={!!errors.exercises?.[exerciseIndex]?.sets?.[setIndex]?.reps}
                    helperText={errors.exercises?.[exerciseIndex]?.sets?.[setIndex]?.reps?.message}
                    />
                  </Grid>
                    <Grid item xs={12} sm={3}>
                      <Button type="button" className="button-style" onClick={() => handleRemoveSet(exerciseIndex, setIndex)}>Remove Set</Button>
                    </Grid>
                </Grid>
              ))}
              <Grid item xs={12}>
                      <Button type="button" className="button-style" onClick={() => handelAddSet(exerciseIndex)}>Add Set</Button>
                    </Grid>
              <Grid item xs={12}>
                <Button type="button" className="button-style" onClick={() => handleRemoveExercise(exerciseIndex)}>
                  Remove Exercise
                </Button>
              </Grid>
            </Grid>
          ...
          <Grid item xs={12}>
            <Button className="button-style" onClick={handleAddExercise}>Add Exercise</Button>
          </Grid>
          <Grid item xs={12}>
            <Button type="submit" variant="contained" color="primary" className="button-style">
              Add Workout
            </Button>
          </Grid>
        </Grid>
